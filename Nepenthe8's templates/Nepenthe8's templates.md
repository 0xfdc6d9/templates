...
例如，$\mathcal z(abacaba) = [0, 0, 1, 0, 3, 0, 1]$
...
> 相比于设 $f[i][j]$ 表示区间 $[L, R]$ 内的最值，ST 表的设计方式所得到的区间体现在数轴上会相对稀疏。
>
> 因为牺牲了一部分区间的信息，所以使用 ST 表时需要满足可重复贡献的性质。

以区间最值为例，设 $f[i][j]$ 表示当前区间的 $[i, i + 2^j - 1]$ 内的最值，显然 $f[i][0] = max[i][i] = num_i$ 。

由倍增思想可得，跳 $2^i$ 步相当于先跳 $2^{i - 1}$ 步再跳 $2^{i - 1}$ 步；同理区间 $[i, i+2^j - 1]$ 内的最值相当于是区间 $[i, i + 2^{j - 1} - 1]$ 和 $[i + 2 ^ {j - 1}, i + 2^j - 1]$ 内的最值。（区间长度段一半）

所以可得式子 $f[i][j] = max(f[i][j - 1], f[i + 2^{j - 1}][j - 1])$ 。

则只需要枚举起点（也就是枚举 $i$ ），接着枚举区间长度（也就是枚举 $j$ ），使得整个区间被包进去，就可以构建出 ST 表了。

ST表常用于解决**可重复贡献问题**。即满足形如 $f(a, a) = a$ 的问题。

常见的可重复贡献问题有：区间最值、区间按位和、区间按位或、区间 GCD 等。而像区间和这样的问题就不是可重复贡献问题。

设有 $n$ 个数、$m$ 个询问，ST 表预处理的时间复杂度为 $\mathcal O(n \log n)$，空间复杂度为 $\mathcal O(n \log n)$，，查询的时间复杂度为 $\mathcal O(m)$。
...
2. 使用 floyd 算法，当最外层枚举中转点到 k 时（尚未开始第 $k$ 次循环），此时最路路径数组 $d$ 还没有使用 $k$ 更新过，那么此时 $d_{ij}$ 就是未经过点k的最短距离。
3. 循环全部点，更新 ans 最小值。
...
那么我们可以将多重背包的问题转化为 01 背包求解：把第i种物品换成 $s_i$ 件 01 背包的物品，每件物品的体积为 $k*v_i$ ，价值为 $k*w_i$ 。($0 \leq k \leq s_i$)。这样的时间复杂度为 $O(m \sum s_i)$。

系数 $k$ 相当于件数，由于二进制拆分可以得到 $0 - s_i$ 范围的数，那么就考虑拆分后的物品取和不取，等价于考虑取 $0 - s_i$ 个原物品。

#### 二进制优化

如果通过二进制拆分第$i$件物品，则可以转化成更少数量的 01 背包的物品，缩小问题规模。
...
迭代加深：在 DFS 的搜索里面，可能会面临一个答案的层数很低，但是 DFS 搜到了另为一个层数很深的分支里面导致时间很慢，但是又卡 BFS 的空间，这个时候，可以限制 DFS 的搜索层数，一次又一次放宽搜索的层数，直到搜索到答案就退出，时间可观，结合了 BFS 和 DFS 的优点于一身。

定义函数：
$$
F ( x ) = G ( x ) + H ( x )
$$
其中 G(x) 表示已经走过的路，H(x) 表示期望走的路。

IDA\* 就是将 F(x) 搬到了 IDDFS（迭代加深的 DFS）上然后加了个可行性剪枝。

估值函数的确定：

松弛问题，求松弛问题后原问题的解。也就是去除掉原问题的一些约束。
...
如果你要查找从一个位置到另一个位置，或者从一个位置到多个目标位置中最近的位置，请使用贪心最佳优先搜索算法或 A\*。在大多数情况下建议使用 A\*。当你使用贪心最佳优先算法时，请考虑使用带有“不可接受的”启发式算法的 A\*。
...
: 因为是直接使用.exe，所以在对拍前先运行一遍写出来的程序，保证当前的.exe文件是最新生成的。
~~~
...
使用时重写 f 函数和随机生成的点 Node 即可，注意多次调用 SA 来减小误差。如果求全局最小值则 $de < 0$ 时接受当前解，如果求全局最大值则 $de>0$ 时接受当前解。
...
~~~c++
constexpr int P = 1000000007; //998244353
// assume -P <= x < 2P
int norm(int x) {
    if (x < 0) {
        x += P;
